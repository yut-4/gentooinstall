...
from gentooinstall.lib.models.application import Audio, AudioConfiguration
...
class AudioApp:
	...
	def _enable_pipewire(
		...
		for user in users:
			...
			install_session.run_in_chroot(f'chown -R {user.username}:{user.username} /home/{user.username}')

			# symlink in the correct pipewire systemd items
			install_session.run_in_chroot(
