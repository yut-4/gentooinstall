...
from gentooinstall.lib.boot import Boot
from gentooinstall.lib.command import SysCommand, run
from gentooinstall.lib.disk.fido import Fido2
from gentooinstall.lib.disk.lvm import lvm_import_vg, lvm_pvseg_info, lvm_vol_change
from gentooinstall.lib.disk.utils import (
...
from gentooinstall.lib.exceptions import DiskError, HardwareIncompatibilityError, RequirementError, ServiceException, SysCallError
from gentooinstall.lib.hardware import SysInfo
from gentooinstall.lib.locale.utils import verify_keyboard_layout, verify_x11_keyboard_layout
from gentooinstall.lib.luks import Luks2, unlock_luks2_dev
from gentooinstall.lib.mirrors import MirrorListHandler
from gentooinstall.lib.models.application import ZramAlgorithm
...
from gentooinstall.lib.output import debug, error, info, log, logger, warn
from gentooinstall.lib.packages.packages import installed_package
from gentooinstall.lib.pacman.config import PacmanConfig
from gentooinstall.lib.pacman.pacman import Pacman
from gentooinstall.lib.plugins import plugins
...
class Installer:
	def __init__(
		...
		It also wraps :py:func:`~gentooinstall.Installer.pacstrap` among other things.
		"""
	...
	def __exit__(self, exc_type: type[BaseException] | None, exc_value: BaseException | None, traceback: TracebackType | None) -> bool | None:
		if exc_type is not None:
			...
			print(tr('Please submit this issue (and file) to https://github.com/gentooinstall/gentooinstall/issues'))
		...
		if not (missing_steps := self.post_install_check()):
			...
			warn('Submit this zip file as an issue to https://github.com/gentooinstall/gentooinstall/issues')
	...
	def _verify_service_stop(self, offline: bool, skip_ntp: bool, skip_wkd: bool) -> None:
		...
		if not skip_ntp:
			...
			while True:
				if not notified and time.time() - started_wait > 5:
					notified = True
					warn(tr('Time synchronization not completing, while you wait - check the docs for workarounds: https://gentooinstall.readthedocs.io/'))
		...
		if not skip_wkd:
			info(tr('Skipping distribution keyring sync wait.'))
	...
	def set_timezone(self, zone: str) -> bool:
		...
		if (Path('/usr') / 'share' / 'zoneinfo' / zone).exists():
			...
			self.run_in_chroot(f'ln -s /usr/share/zoneinfo/{zone} /etc/localtime')
			return True
	...
	def run_in_chroot(self, cmd: str, run_as: str | None = None, peek_output: bool = False) -> SysCommand:
		if run_as:
	...
	def copy_iso_network_config(self, enable_services: bool = False) -> bool:
		...
		# For further details see  https://wiki.gentoo.org/title/Systemd-resolved#DNS
	...
	def mkinitcpio(self, flags: list[str]) -> bool:
		...
		try:
			self.run_in_chroot(f'mkinitcpio {" ".join(flags)}', peek_output=True)
			return True
		except SysCallError as e:
			if e.worker_log:
	...
	def _prepare_fs_type(
		...
		# https://github.com/gentooinstall/gentooinstall/issues/1837
	...
	def minimal_installation(
		...
		if ucode := self._get_microcode():
			...
			debug('gentooinstall will not install any ucode.')
		...
		#
		# https://github.com/gentooinstall/gentooinstall/issues/880
		# https://github.com/gentooinstall/gentooinstall/issues/1837
		# https://github.com/gentooinstall/gentooinstall/issues/1841
		if not self._disable_fstrim:
			self.enable_periodic_trim()

		# TODO: Support locale and timezone
		# os.remove(f'{self.target}/etc/localtime')
		# sys_command(f'chroot {self.target} ln -s /usr/share/zoneinfo/{localtime} /etc/localtime')
		# sys_command('chroot /mnt hwclock --hctosys --localtime')
		if hostname:
		...
		if snapshot_type == SnapshotType.Snapper:
			...
			for config_name, mountpoint in snapper.items():
				try:
					...
					self.run_in_chroot(' '.join(shlex.quote(arg) for arg in snapper_cmd), peek_output=True)
				except SysCallError as err:
	...
	def setup_swap(self, kind: str = 'zram', algo: ZramAlgorithm = ZramAlgorithm.ZSTD) -> None:
		if kind == 'zram':
			...
			raise ValueError('gentooinstall currently only supports setting up swap on zram')
	...
	def _get_kernel_params_partition(
		...
		if root_partition in self._disk_encryption.partitions:
			...
			if self._disk_encryption.hsm_device:
				...
				# https://github.com/gentooinstall/gentooinstall/pull/1196#issuecomment-1129715645
	...
	def _get_kernel_params(
		...
		# https://github.com/gentooinstall/gentooinstall/issues/881
	...
	def _add_systemd_bootloader(
		...
		# TODO: This is a temporary workaround to deal with https://github.com/gentooinstall/gentooinstall/pull/3396#issuecomment-2996862019
		...
		try:
			# Force EFI variables since bootctl may not detect them inside chroot environments
			...
			if systemd_version >= '258':
				self.run_in_chroot(f'bootctl --variables=yes {" ".join(bootctl_options)} install')
			else:
				self.run_in_chroot(f'bootctl {" ".join(bootctl_options)} install')
		except SysCallError:
			if systemd_version >= '258':
				# Fallback, try creating the boot loader without touching the EFI variables
				self.run_in_chroot(f'bootctl --variables=no {" ".join(bootctl_options)} install')
			else:
				self.run_in_chroot(f'bootctl --no-variables {" ".join(bootctl_options)} install')
		...
		if uki_enabled:
			default_entry = f'gentoo-{default_kernel}.efi'
		else:
	...
	def _add_grub_bootloader(
		...
		if SysInfo.has_uefi():
			...
			try:
				full_command = command + add_options
				self.run_in_chroot(' '.join(shlex.quote(arg) for arg in full_command), peek_output=True)
			except SysCallError as err:
	...
	def _add_limine_bootloader(
		...
		if SysInfo.has_uefi():
			...
			try:
				...
				if bootloader_removable:
					...
					efi_dir_path = efi_dir_path / 'gentoo-limine'
					efi_dir_path_target = efi_dir_path_target / 'gentoo-limine'
			...
			if not bootloader_removable:
				...
				if efi_bitness == '64':
					loader_path = '\\EFI\\gentoo-limine\\BOOTX64.EFI'
				elif efi_bitness == '32':
					loader_path = '\\EFI\\gentoo-limine\\BOOTIA32.EFI'
				else:
			...
			try:
				...
				self.run_in_chroot(f'limine bios-install {parent_dev_path}', peek_output=True)
			except Exception as err:
		...
		for kernel in self.kernels:
			if uki_enabled:
				entry = [
					'protocol: efi',
					f'path: boot():/EFI/Linux/gentoo-{kernel}.efi',
	...
	def _add_efistub_bootloader(
		...
		if not uki_enabled:
			...
			loader = '/EFI/Linux/gentoo-{kernel}.efi'
			cmdline = []
	...
	def _add_refind_bootloader(
		...
		try:
			self.run_in_chroot('refind-install')
		except SysCallError as err:
		...
		for kernel in self.kernels:
			if uki_enabled:
				entry = f'"Gentoo Linux ({kernel}) UKI" "{kernel_params}"'
			else:
				if boot_on_root:
				...
				entry = f'"Gentoo Linux ({kernel})" "{kernel_params} {initrd_path}"'
	...
	def add_bootloader(self, bootloader: Bootloader, uki_enabled: bool = False, bootloader_removable: bool = False) -> None:
		...
		The installer supports one of five types:
		* systemd-bootctl
		* grub
		* limine
		* efistub (beta)
	...
	def _create_user(self, user: User) -> None:
		...
		for group in user.groups:
			self.run_in_chroot(f'gpasswd -a {user.username} {group}')
	...
	def user_set_shell(self, user: str, shell: str) -> bool:
		...
		try:
			self.run_in_chroot(f'sh -c "chsh -s {shell} {user}"')
			return True
		except SysCallError:
			return False
	...
	def chown(self, owner: str, path: str, options: list[str] = []) -> bool:
		...
		try:
			self.run_in_chroot(f"sh -c 'chown {' '.join(options)} {owner} {cleaned_path}'")
			return True
		except SysCallError:
			return False
	...
	def set_vconsole(self, locale_config: LocaleConfiguration) -> None:
		...
		# https://wiki.gentoo.org/title/Linux_console#Fonts
	...
	def set_keyboard_language(self, language: str) -> bool:
		...
		if len(language.strip()):
			...
			# In accordance with https://github.com/gentooinstall/gentooinstall/issues/107#issuecomment-841701968
			# Setting an empty keymap first, allows the subsequent call to set layout for both console and x11.
			with Boot(self.target) as session:
				os.system('systemd-run --machine=gentooinstall --pty localectl set-keymap ""')
...
def run_custom_user_commands(commands: list[str], installation: Installer) -> None:
	for index, command in enumerate(commands):
		...
		installation.run_in_chroot(f'bash {script_path}')
