...
from gentooinstall.lib.utils.encoding import unicode_ljust, unicode_rjust
...
class Journald:
	...
	def log(message: str, level: int = logging.DEBUG) -> None:
		...
		log_adapter = logging.getLogger('gentooinstall')
...
class Logger:
	def __init__(self, path: Path = Path('/var/log/gentooinstall')) -> None:
		self._path = path
