...
from gentooinstall.lib.output import error, info, warn
from gentooinstall.lib.version import get_version

plugins = {}


# 1: List gentooinstall.plugin definitions
# 2: Load the plugin entrypoint
# 3: Initiate the plugin and store it as .name in plugins
for plugin_definition in metadata.entry_points().select(group='gentooinstall.plugin'):
...
# @gentooinstall.plugin decorator hook to programmatically add
...
def load_plugin(path: Path) -> None:
	...
	if namespace and namespace in sys.modules:
		# Version dependency via __gentooinstall__version__ variable in the plugin.
		...
		if version is not None:
			...
			plugin_module = sys.modules[namespace]
			plugin_version = getattr(plugin_module, '__gentooinstall__version__', None)

			if plugin_version is not None and plugin_version < float(version_major_and_minor):
				error(f'Plugin {plugin_module} does not support the current gentooinstall version.')
