...
from gentooinstall.lib.exceptions import DownloadTimeout, SysCallError
...
def update_keyring() -> bool:
	info('Refreshing Gentoo release keyring metadata ...')
	try:
		SysCommand('emerge --sync')
		return True
	except SysCallError:
		if os.geteuid() != 0:
			error("update_keyring() requires root privileges to run 'emerge --sync'.")
	except Exception as err:
		debug(f'Keyring metadata refresh failed: {err}')
...
def ping(hostname: str, timeout: int = 5) -> int:
	...
	started = time.time()
	random_identifier = f'gentooinstall-{random.randint(1000, 9999)}'.encode()

	# Create a raw socket (requires root, which should be fine on live media)
