...
from gentooinstall.lib.command import SysCommand, SysCommandWorker, locate_binary
from gentooinstall.lib.exceptions import SysCallError
from gentooinstall.lib.output import error
...
class Boot:
	...
	def __init__(self, path: Path | str):
		...
		self.path = path
		self.container_name = 'gentooinstall'
		self.session: SysCommandWorker | None = None
		self.ready = False

	def __enter__(self) -> Self:
		if Boot._active_boot and Boot._active_boot.path != self.path:
			raise KeyError('gentooinstall only supports booting up one instance and another session is already active.')
