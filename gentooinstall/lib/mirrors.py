...
from gentooinstall.lib.menu.abstract_menu import AbstractSubMenu
from gentooinstall.lib.menu.helpers import Input, Loading, Selection
from gentooinstall.lib.menu.list_manager import ListManager
from gentooinstall.lib.models.mirrors import (
	CustomRepository,
	CustomServer,
	MirrorConfiguration,
	MirrorRegion,
...
from gentooinstall.lib.output import FormattedOutput, debug, info
...
class MirrorListHandler:
	def __init__(
		self,
		local_mirrorlist: Path = Path('/etc/portage/make.conf'),
		offline: bool = False,
		verbose: bool = False,
	) -> None:
	...
	def load_remote_mirrors(self) -> bool:
		# Gentoo does not provide an official mirror status JSON API in the format
		# used by this installer. Keep remote fetch disabled and rely on local
		# make.conf/custom mirror config instead.
		debug('Remote mirror API is disabled; using local mirror configuration.')
		return False
	...
	def load_local_mirrors(self) -> None:
		if not self._local_mirrorlist.exists():
			self._status_mappings = {
				'Worldwide': [
					MirrorStatusEntryV3(
						url='https://distfiles.gentoo.org/',
						protocol='https',
						active=True,
						country='Worldwide',
						country_code='WW',
		...
		with self._local_mirrorlist.open('r') as fp:
			content = fp.read()
			self._status_mappings = self._parse_local_mirrors(content)
		...
		for mirror in mirror_status.urls:
			...
			if any(
				[
					...
					# mirror.score (error rate) over time reported by upstream backend
			...
			if mirror.country == '':
				# Some entries may miss region metadata in their backend payload.
				# In those cases, treat them as worldwide mirrors.
	...
	def _parse_local_mirrors(self, mirrorlist: str) -> dict[str, list[MirrorStatusEntryV3]]:
		if 'GENTOO_MIRRORS=' in mirrorlist:
			return self._parse_make_conf_mirrors(mirrorlist)
		...
		for line in lines:
			...
			if line.startswith('Server = '):
				...
				mirror_entry = MirrorStatusEntryV3(
					...
					# These values are normally populated by a remote mirror API
					# and cannot be inferred reliably from local files alone.
					country_code='WW',
					isos=True,
					ipv4=True,
					ipv6=True,
	...
	def _parse_make_conf_mirrors(self, make_conf: str) -> dict[str, list[MirrorStatusEntryV3]]:
		mirrors: list[str] = []
		for raw_line in make_conf.splitlines():
			line = raw_line.strip()
			if not line or line.startswith('#') or 'GENTOO_MIRRORS=' not in line:
				continue

			value = line.split('=', 1)[1].strip().strip('"').strip("'")
			mirrors.extend([url for url in value.split() if url.startswith('http')])
