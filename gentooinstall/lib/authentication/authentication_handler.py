...
from gentooinstall.lib.command import SysCommandWorker
...
class AuthenticationHandler:
	...
	def _configure_u2f_mapping(
		...
		for user in users:
			...
			chroot_args = ['pamu2fcfg', '-u', user.username, '-o', f'pam://{hostname}', '-i', f'pam://{hostname}']
			chroot_cmd = ' '.join(shlex.quote(arg) for arg in chroot_args)
			cmd = f'chroot {shlex.quote(str(install_session.target))} /bin/bash -lc {shlex.quote(chroot_cmd)}'
