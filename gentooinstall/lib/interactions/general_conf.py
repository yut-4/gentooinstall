...
from gentooinstall.lib.locale.utils import list_timezones
from gentooinstall.lib.menu.helpers import Confirmation, Input, Selection
from gentooinstall.lib.output import warn
from gentooinstall.lib.translationhandler import Language, tr
...
def select_ntp(preset: bool = True) -> bool:
	...
	header += (
		tr(
			'Hardware time and other post-configuration steps might be required in order for NTP to work.\nFor more information, please check the Gentoo wiki',
		)
		+ '\n'
	)
...
def select_language(preset: str | None = None) -> str | None:
	from gentooinstall.lib.locale.locale_menu import select_kb_layout
...
def select_installer_language(languages: list[Language], preset: Language) -> Language:
...
def add_number_of_parallel_downloads(preset: int = 1) -> int | None:
	...
	make_conf_path = Path('/etc/portage/make.conf')
	if make_conf_path.exists():
		lines = make_conf_path.read_text().splitlines()
		entry = f'EMERGE_DEFAULT_OPTS="--jobs {downloads} --load-average {downloads}"'
		for idx, line in enumerate(lines):
			if line.strip().startswith('EMERGE_DEFAULT_OPTS='):
				lines[idx] = entry
				break
		else:
			lines.append(entry)
		make_conf_path.write_text('\n'.join(lines) + '\n')
