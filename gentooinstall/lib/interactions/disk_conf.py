...
from gentooinstall.lib.disk.partitioning_menu import manual_partitioning
from gentooinstall.lib.menu.helpers import Confirmation, Notify, Selection, Table
from gentooinstall.lib.menu.util import prompt_dir
...
from gentooinstall.lib.output import FormattedOutput, debug
	...
	match result.type_:
		...
		case ResultType.Selection:
			...
			if selection == pre_mount_mode:
				...
				output += tr("WARNING: gentooinstall won't check the suitability of this setup")
...
def get_default_btrfs_subvols() -> list[SubvolumeModification]:
	...
	# Default layout inspired by common Btrfs workstation setups.
	return [
		SubvolumeModification(Path('@'), Path('/')),
		SubvolumeModification(Path('@home'), Path('/home')),
		SubvolumeModification(Path('@log'), Path('/var/log')),
		SubvolumeModification(Path('@distfiles'), Path('/var/cache/distfiles')),
	]
...
def suggest_single_disk_layout(
	...
	# Used for reference: https://wiki.gentoo.org/wiki/Partitioning_a_disk
...
def suggest_multi_disk_layout(
	...
	if home_device is None or root_device is None:
		...
		text += tr('Minimum capacity for Gentoo Linux partition: {}GiB').format(desired_root_partition_size.format_size(Unit.GiB))
...
def suggest_lvm_layout(
	disk_config: DiskLayoutConfiguration,
	filesystem_type: FilesystemType | None = None,
	vg_grp_name: str = 'GentooInstallVg',
) -> LvmConfiguration:
