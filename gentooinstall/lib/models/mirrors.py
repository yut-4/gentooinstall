...
from gentooinstall.lib.networking import DownloadTimer, ping
	...
	def speed(self) -> float:
		if self._speed is None:
			...
			while retry < self._speedtest_retries and self._speed is None:
				probe_url = f'{self.url}core/os/x86_64/core.db'
				if '$repo' not in self.url:
					probe_url = urllib.parse.urljoin(self.server_url, 'metadata/timestamp.chk')

				debug(f'Checking download speed of {self._hostname}[{self.score}] by fetching: {probe_url}')
				req = urllib.request.Request(url=probe_url)
...
class MirrorStatusListV3(BaseModel):
	...
	def check_model(
		...
		raise ValueError('MirrorStatusListV3 only accepts version 3 mirror status payloads.')
